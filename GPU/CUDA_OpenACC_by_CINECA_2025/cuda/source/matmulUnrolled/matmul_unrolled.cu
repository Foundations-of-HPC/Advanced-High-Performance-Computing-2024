...
__global__ void matmul_kernel_unrolled(float *A, float *B, float *C, int M, int N, int K) {
...
int main() {
    ...
    printf("\n=== Memory Strategy: PINNED MEMORY ===\n");
    printf("1. Using cudaMallocHost() for pinned (page-locked) memory\n");
    printf("2. Enables faster host-device transfers\n");
    printf("3. Allows asynchronous memory operations\n");
    printf("4. Better performance compared to pageable memory\n");
    ...
    printf("=== CUDA Matrix Multiplication with Pinned Memory Transfer ===\n");
    ...
    for (int test = 0; test < num_sizes; test++) {
        ...
        // Allocate pinned host (CPU) memory
        ...
        CUDA_CHECK(cudaMallocHost(&h_C_gpu, M * N * sizeof(float)));
        CUDA_CHECK(cudaMallocHost(&h_C_cpu, M * N * sizeof(float)));
        ...
        // Time data transfer: Host to Device (using pinned memory)
        printf("Transferring data from Host to Device (pinned memory)...\n");
        ...
        // Warm up GPU
        matmul_kernel_unrolled<<<gridSize, blockSize>>>(d_A, d_B, d_C, M, N, K);
        ...
        // Time data transfer: Device to Host (using pinned memory)
        printf("Transferring results from Device to Host (pinned memory)...\n");
        CUDA_CHECK(cudaEventRecord(transfer_start));
