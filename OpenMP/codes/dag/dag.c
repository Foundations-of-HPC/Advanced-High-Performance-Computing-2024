...
//
// this is the entry point for the dag processing
 ...
 #pragma omp parallel
  ...
  for ( uint n = 0; n < p->N-2; n++ )
    ...
    //
    {
      ...
      // min_children leaves (just a choice, nothing profound
	...
	// if there are not enough nodes left,
	// we just cut the number of children.
	//
  ...
  if ( cidx < p->N * p->max_children )
	...
	// we need to re-direct the pointers
	// only if the allocation region has changed;
	// normally that does not happen when you *reduce*
	// the allocation.
	//
  	{
