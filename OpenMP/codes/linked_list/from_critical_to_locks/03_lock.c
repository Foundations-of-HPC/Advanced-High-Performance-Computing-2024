...
typedef struct {
  ...
  // NOTE: this structure is small, many could fit in the same cache line
  //       if two threads operate on two different structures that reside
  //       in the same cache line, that will result in a false sharing and
  //       in a performance degradation; general guideline: better to align
  //       the structure to the cache line
 } part_t;
