			      ...
			      llnode_t **new_node )
{
  ...
  while ( !done )
      ...
      else
	{
	  ...
	  // allocate a new node
	  // NOTE:: a malloc should not be inside a "critical" section;
	  //        allocating memory is expensive and often requirea a "global lock"
	  //        we should allocate the node optimistically at the very entry
	  //        point of the routine
	  // HINT: transform everything in an array of nodes; sparse memory allocation
	  //       is BAD
	  //
