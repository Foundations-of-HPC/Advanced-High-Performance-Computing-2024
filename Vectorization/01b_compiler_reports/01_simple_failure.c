                ...
                double * c, int n)
{
  /* for GCC */
  /*
  __attribute__(( __assume__((size_t)a + n*sizeof(double) <= (size_t)b ||
			     (size_t)b + n*sizeof(double) <= (size_t)a) ));
  __attribute__(( __assume__((size_t)a + n*sizeof(double) <= (size_t)c ||
			     (size_t)c + n*sizeof(double) <= (size_t)a) ));
  */
  /* for CLANG */
  /*
  __builtin_assume((size_t)a + n*sizeof(double) <= (size_t)b ||
		   (size_t)b + n*sizeof(double) <= (size_t)a);
  __builtin_assume((size_t)a + n*sizeof(double) <= (size_t)c ||
		   (size_t)c + n*sizeof(double) <= (size_t)a);
  */
